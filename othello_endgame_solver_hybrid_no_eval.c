/**
 * @file othello_endgame_solver_hybrid_no_eval.c
...
// 出力制御フラグ
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// MINIMAL_OUTPUT=1: 本番用（Total行と最終結果のみ出力）
// MINIMAL_OUTPUT=0: デバッグ用（詳細情報を出力）
...
static void debug_init(const char *log_filename, bool verbose, bool track_threads,
                      bool track_tree, bool real_time, bool track_ws) {
...
static inline uint64_t get_moves_scalar(uint64_t P, uint64_t O) {
    ...
    pre_l = mask & (flip_l << 1);
    pre_r = mask & (flip_r >> 1);
    flip_l |= pre_l | (mask & (pre_l << 2));
    flip_r |= pre_r | (mask & (pre_r >> 2));
    flip_l |= mask & (flip_l << 1);
    flip_r |= mask & (flip_r >> 1);
...
// ────────────────────────────────────────────────────────────
// ロックフリーTT設定
// USE_LOCKFREE_TT=1 でロックフリー版を使用（384スレッド以上推奨）
// USE_LOCKFREE_TT=0 または未定義で従来のRWロック版を使用
// ────────────────────────────────────────────────────────────
#ifndef USE_LOCKFREE_TT
#define USE_LOCKFREE_TT 0
#endif

// ────────────────────────────────────────────────────────────
// DFPN閾値計算設定
// USE_DFPN_THRESHOLD=1 で標準DFPN閾値計算を使用（探索効率向上）
// USE_DFPN_THRESHOLD=0 または未定義で従来版（INF+1固定）を使用
// ────────────────────────────────────────────────────────────
#ifndef USE_DFPN_THRESHOLD
#define USE_DFPN_THRESHOLD 0
#endif
...
//
// 【設計】
//   2つの64bitワードをアトミックに読み書き:
//   - key_xor: key ^ data（データ破損検出用）
//   - data: pn(32bit) | dn(32bit)
//
// 【検証方法】
//   読み取り時: (key_xor ^ data) == key ならば有効
//   部分書き込みによるデータ破損を検出可能
//
// 【制限】
//   - result, depth, eval_score は保存しない（簡略化）
//   - 証明済み(pn=0 or dn=0)の情報のみで判定
//

typedef struct {
    _Atomic uint64_t key_xor;   // key ^ data (検証用)
    _Atomic uint64_t data;      // pn(下位32bit) | dn(上位32bit)
} TTEntryLockFree;

typedef struct {
    TTEntryLockFree *entries;
    size_t size;
    size_t mask;
...
#endif // USE_LOCKFREE_TT

// Stripe lock configuration for TT (RWロック版で使用)
...
static inline uint64_t hash_position(uint64_t P, uint64_t O) {
    ...
    // 【問題点】
    //   - 64回のループで128回の条件分岐
    //   - 石は平均30個程度なのに全64マスをチェック
    //
    // 【改善内容】
    //   - __builtin_ctzll でセットビットの位置を直接取得
    //   - bits &= bits - 1 で最下位ビットをクリア
    //   - ループ回数 = 石の数（約30回）に削減
    //
    // 【期待効果】
    //   - 約2倍の高速化
    ...
    // Player (黒石) のセットビットのみ処理
    bits = unique_P;
    while (bits) {
        int i = __builtin_ctzll(bits);  // 最下位セットビットの位置
        hash ^= zobrist_table[0][i];
        bits &= bits - 1;               // 最下位ビットをクリア
    }

    // Opponent (白石) のセットビットのみ処理
    bits = unique_O;
    while (bits) {
        int i = __builtin_ctzll(bits);
        hash ^= zobrist_table[1][i];
        bits &= bits - 1;
    }
...
static TranspositionTable* tt_create(size_t size_mb) {
    ...
    size_t entry_size = sizeof(TTEntryLockFree);
    ...
    tt->size = size;
    tt->mask = size - 1;
    tt->entries = calloc(size, sizeof(TTEntryLockFree));

    atomic_store(&tt->hits, 0);
    atomic_store(&tt->stores, 0);
    atomic_store(&tt->collisions, 0);

    debug_log("TT created (LOCK-FREE): %zu MB (%zu entries)\n", size_mb, size);
...
static bool tt_probe(TranspositionTable *tt, uint64_t key, int depth,
                    uint32_t *pn, uint32_t *dn, Result *result) {
    (void)depth;  // ロックフリー版ではdepthは使用しない

    size_t index = key & tt->mask;
    TTEntryLockFree *entry = &tt->entries[index];

    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // ロックフリー読み取り（ダブルチェック方式）
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // 【手順】
    //   1. key_xorを読む
    //   2. dataを読む
    //   3. key_xorを再度読む
    //   4. 両方のkey_xorが一致すれば有効
    //
    // 【理由】
    //   書き込み順序: data → key_xor
    //   もし読み取り中に書き込みが入った場合:
    //   - key_xor_1 != key_xor_2 となり無効と判定
    //   - 不整合なデータ使用を防止

    uint64_t key_xor_1 = atomic_load_explicit(&entry->key_xor, memory_order_acquire);
    uint64_t stored_data = atomic_load_explicit(&entry->data, memory_order_acquire);
    uint64_t key_xor_2 = atomic_load_explicit(&entry->key_xor, memory_order_acquire);

    // ダブルチェック: 両方のkey_xor読み取りが一致しなければ無効
    if (key_xor_1 != key_xor_2) {
        // 読み取り中に書き込みがあった
        return false;
    }

    // キー検証: key_xor ^ data == key ならば有効なエントリ
    uint64_t reconstructed_key = key_xor_1 ^ stored_data;

    if (reconstructed_key == key && stored_data != 0) {
        // データをアンパック
        *pn = (uint32_t)(stored_data & 0xFFFFFFFFULL);
        *dn = (uint32_t)(stored_data >> 32);

        // resultはpn/dnから推測
        if (*pn == 0) {
            *result = RESULT_EXACT_WIN;
        } else if (*dn == 0) {
            *result = RESULT_EXACT_LOSE;
        } else {
            *result = RESULT_UNKNOWN;
        }

        atomic_fetch_add_explicit(&tt->hits, 1, memory_order_relaxed);
        return true;
    }

    if (stored_data != 0) {
        atomic_fetch_add_explicit(&tt->collisions, 1, memory_order_relaxed);
    }
...
static void tt_store(TranspositionTable *tt, uint64_t key, int depth,
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // 【方針】
    //   証明済みエントリ（pn=0 or dn=0）は中間結果で上書きしない
    //   同一キーの場合は常に更新（より新しい情報）
    //   異なるキーの場合は「証明済み > 中間結果」で判定

    // 現在の格納値を読み取り
    uint64_t old_key_xor = atomic_load_explicit(&entry->key_xor, memory_order_acquire);
    uint64_t old_data = atomic_load_explicit(&entry->data, memory_order_acquire);

    if (old_data != 0) {
        uint64_t old_key = old_key_xor ^ old_data;
        uint32_t old_pn = (uint32_t)(old_data & 0xFFFFFFFFULL);
        uint32_t old_dn = (uint32_t)(old_data >> 32);
        bool old_proven = (old_pn == 0 || old_dn == 0);
        bool new_proven = (pn == 0 || dn == 0);

        if (old_key == key) {
            // 同一キー: より良い情報（証明済み or 小さいpn/dn）なら更新
            if (old_proven && !new_proven) {
                // 証明済みを中間結果で上書きしない
                return;
            }
        } else {
    ...
    // データをパック: pn(下位32bit) | dn(上位32bit)
    uint64_t data = ((uint64_t)dn << 32) | (uint64_t)pn;

    // key_xor = key ^ data（検証用）
    uint64_t key_xor = key ^ data;

    // アトミック書き込み（データを先に、key_xorを後に）
    atomic_store_explicit(&entry->data, data, memory_order_release);
    atomic_store_explicit(&entry->key_xor, key_xor, memory_order_release);

    atomic_fetch_add_explicit(&tt->stores, 1, memory_order_relaxed);
}
...
static TranspositionTable* tt_create(size_t size_mb) {
    ...
    debug_log("TT created (RW-LOCK): %zu MB (%zu entries)\n", size_mb, size);
...
static DFPNNode* select_best_child_with_priority(DFPNNode *node) {
    ...
    if (node->type == NODE_OR) {
        // ORノード: pnが最小の子を選択（より証明しやすい）
        for (int i = 0; i < node->n_children; i++) {
            DFPNNode *child = node->children[i];
            if (child->pn == 0 || child->dn == 0) continue;
            if (child->is_proven) continue;

            int priority = (int)(PN_INF - child->pn);
        ...
        // ANDノード: dnが最小の子を選択（より反証しやすい）
        for (int i = 0; i < node->n_children; i++) {
            DFPNNode *child = node->children[i];
            if (child->pn == 0 || child->dn == 0) continue;
            if (child->is_proven) continue;

            int priority = (int)(DN_INF - child->dn);
            if (priority > best_priority) {
                best_priority = priority;
                best_idx = i;
            }
...
#if USE_DFPN_THRESHOLD
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// DFPN閾値計算
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//
// 【標準DFPN閾値計算】
//   子ノードに閾値を伝播させて無駄な探索を削減する
//
// 【ORノード】 pn = min(子のpn), dn = sum(子のdn)
//   - child_threshold_pn = min(parent_threshold_pn, second_min_pn + 1)
//   - child_threshold_dn = parent_threshold_dn - parent_dn + child_dn
//
// 【ANDノード】 pn = sum(子のpn), dn = min(子のdn)
//   - child_threshold_pn = parent_threshold_pn - parent_pn + child_pn
//   - child_threshold_dn = min(parent_threshold_dn, second_min_dn + 1)
//
// 【効果】
//   - 証明に不要な子ノードの深い探索を回避
//   - 特に384スレッド以上の環境で効果大
//
static DFPNNode* select_best_child_with_threshold(DFPNNode *node) {
    if (!node->children || node->n_children == 0) return NULL;

    // 最良の子とsecond-bestを同時に探索
    int best_idx = -1;
    uint32_t best_value = UINT32_MAX;
    uint32_t second_best_value = UINT32_MAX;
    ...
    if (node->type == NODE_OR) {
        ...
        for (int i = 0; i < node->n_children; i++) {
            ...
            if (child->pn < best_value) {
                second_best_value = best_value;
                best_value = child->pn;
                best_idx = i;
            } else if (child->pn < second_best_value) {
                second_best_value = child->pn;
            }
        }

        if (best_idx < 0) return NULL;

        DFPNNode *best_child = node->children[best_idx];

        // DFPN閾値計算（ORノード）
        // threshold_pn = min(parent_threshold_pn, second_min_pn + 1)
        uint32_t new_threshold_pn;
        if (second_best_value == UINT32_MAX) {
            // 子が1つしかない場合
            new_threshold_pn = node->threshold_pn;
        } else {
            uint32_t second_plus_one = (second_best_value < PN_INF) ? second_best_value + 1 : PN_INF;
            new_threshold_pn = (node->threshold_pn < second_plus_one) ? node->threshold_pn : second_plus_one;
        }

        // threshold_dn = parent_threshold_dn - parent_dn + child_dn
        uint64_t new_threshold_dn_64 = (uint64_t)node->threshold_dn - (uint64_t)node->dn + (uint64_t)best_child->dn;
        uint32_t new_threshold_dn = (new_threshold_dn_64 > DN_INF) ? DN_INF + 1 : (uint32_t)new_threshold_dn_64;

        best_child->threshold_pn = new_threshold_pn;
        best_child->threshold_dn = new_threshold_dn;
        ...
        for (int i = 0; i < node->n_children; i++) {
            ...
            if (child->dn < best_value) {
                second_best_value = best_value;
                best_value = child->dn;
                best_idx = i;
            } else if (child->dn < second_best_value) {
                second_best_value = child->dn;
            }
        }
        ...
        // DFPN閾値計算（ANDノード）
        // threshold_pn = parent_threshold_pn - parent_pn + child_pn
        uint64_t new_threshold_pn_64 = (uint64_t)node->threshold_pn - (uint64_t)node->pn + (uint64_t)best_child->pn;
        uint32_t new_threshold_pn = (new_threshold_pn_64 > PN_INF) ? PN_INF + 1 : (uint32_t)new_threshold_pn_64;

        // threshold_dn = min(parent_threshold_dn, second_min_dn + 1)
        uint32_t new_threshold_dn;
        if (second_best_value == UINT32_MAX) {
            // 子が1つしかない場合
            new_threshold_dn = node->threshold_dn;
        } else {
            uint32_t second_plus_one = (second_best_value < DN_INF) ? second_best_value + 1 : DN_INF;
            new_threshold_dn = (node->threshold_dn < second_plus_one) ? node->threshold_dn : second_plus_one;
        }
...
static void update_pn_dn(DFPNNode *node) {
    ...
    if (node->type == NODE_OR) {
        ...
        for (int i = 0; i < node->n_children; i++) {
            ...
            // ★ Fix: 証明済みDRAWの子のdnはsum計算に含めない
            // DRAWはdn=INFなので、そのまま足すと他の子を探索できなくなる
            if (child->is_proven && child->result == RESULT_EXACT_DRAW) {
                // DRAWの子は探索対象外なので、dnには固定値1を足す
                sum_dn += 1;
            } else {
                sum_dn += child->dn;
            }
        ...
        if (node->pn == 0) {
            ...
            // Note: With DRAW=LOSE treatment, this branch rarely executes
        ...
        for (int i = 0; i < node->n_children; i++) {
            ...
            // ★ Fix: 証明済みDRAWの子のpnはsum計算に含めない
            // DRAWはpn=INFなので、そのまま足すと他の子を探索できなくなる
            if (child->is_proven && child->result == RESULT_EXACT_DRAW) {
                // DRAWの子は探索対象外なので、pnには固定値1を足す
                sum_pn += 1;
            } else {
                sum_pn += child->pn;
            }
...
static void expand_node(Worker *worker, DFPNNode *node) {
    ...
    // 子ノードを作成（評価関数なしで単純に展開）
    int idx = 0;
    while (moves) {
        int move = first_one(moves);
        moves &= moves - 1;
...
static void dfpn_solve_node(Worker *worker, DFPNNode *node) {
    ...
    if (tt_probe(tt, key, node->depth, &node->pn, &node->dn, &node->result)) {
    ...
    if (node->children == NULL) {
        ...
        if (node->n_children == 0) {
            ...
            // DRAW handling: Keep pn=INF, dn=INF for proper propagation
            // DRAW will be treated as LOSE in final result output (WPNS compatible)
            if (node->type == NODE_OR) {
                ...
                if (score > 0) {
                    ...
                    // DRAW: pn=INF, dn=INF for propagation
                    node->result = RESULT_EXACT_DRAW;
                    node->pn = PN_INF;
                    node->dn = DN_INF;
                }
            ...
            tt_store(worker->global->tt, key, node->depth, node->pn, node->dn, node->result);
    ...
    if (node->children != NULL && node->n_children > 1) {
        ...
        if (shared_has_space && (local_heap_needs_fill || (has_idle && idle_rate > 0.5f) || node->depth >= worker->global->min_depth_for_spawn)) {
            ...
            for (int i = 1; i < node->n_children && early_spawned < max_early_spawn; i++) {
                ...
                int priority;
                if (node->type == NODE_OR) {
                    priority = (PN_INF - child->pn) / 1000;
                } else {
                    priority = (DN_INF - child->dn) / 1000;
                }
    ...
    while(node->pn > 0 && node->dn > 0 &&
        ...
        if (loop_count >= 50 && node->children != NULL && node->n_children > 1) {
            ...
            if (has_idle && node->pn > 0 && node->dn > 0) {
                ...
                for (int i = 0; i < node->n_children && spawned < 2; i++) {
                    ...
                    int priority;
                    if (node->type == NODE_OR) {
                        priority = (PN_INF - c->pn) / 1000;
                    } else {
                        priority = (DN_INF - c->dn) / 1000;
                    }
        ...
        // DFPN閾値計算を使用（探索効率向上）
        // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        DFPNNode *child = select_best_child_with_threshold(node);
        if (child == NULL) break;
        // threshold_pn/dn は select_best_child_with_threshold 内で設定済み
#else
        // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        // 従来版: INF+1固定（完全探索）
        // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        ...
        if (child == NULL) break;
        child->threshold_pn = PN_INF + 1;
        child->threshold_dn = DN_INF + 1;
#endif
...
// Forward declaration for process_task (needed by process_root_task_with_split)
static bool process_task(Worker *worker, Task *task);

static bool process_root_task_with_split(Worker *worker, Task *task) {
    // ★ ROOT_SPLITを一時的に無効化：通常のprocess_taskに委譲
    // サブタスクの結果が親に戻らない問題があるため
    task->generation = 1;  // generation=1にして通常処理へ
    return process_task(worker, task);

#if 0  // ROOT_SPLIT disabled - causes infinite loop
    printf("DEBUG: Entering ROOT_SPLIT for move %c%d\n",
    ...
    root->player = p;
    root->opponent = o;
    root->type = NODE_OR;   // task->playerが手番を持っているのでORノード
    ...
    if (root->children == NULL || root->n_children == 0) {
        ...
        // 結果判定（TT用=ツリー視点、move_result=元プレイヤー視点で反転）
        uint64_t key = hash_position(p, o);
        Result tt_result = RESULT_UNKNOWN;    // TTに保存（ツリー視点、非反転）
        Result move_result = RESULT_UNKNOWN;  // move_results用（元プレイヤー視点、反転）
        if (root->pn == 0) {
            tt_result = RESULT_EXACT_WIN;     // ツリー視点: pn=0はWIN
            move_result = RESULT_EXACT_LOSE;  // 元プレイヤー視点: 反転してLOSE
        } else if (root->dn == 0) {
            tt_result = RESULT_EXACT_LOSE;    // ツリー視点: dn=0はLOSE
            move_result = RESULT_EXACT_WIN;   // 元プレイヤー視点: 反転してWIN
        }
        tt_store(worker->global->tt, key, root->depth, root->pn, root->dn, tt_result);

        // WIN報告（反転後のmove_result）
        if (move_result == RESULT_EXACT_WIN) {
    ...
    if (root->pn == 0) {
        ...
        tt_result = RESULT_EXACT_LOSE;    // ツリー視点: dn=0はLOSE
        move_result = RESULT_EXACT_WIN;   // 元プレイヤー視点: 反転してWIN
    } else if (root->pn >= PN_INF && root->dn >= DN_INF && root->is_proven) {
        // ★ Fix: Only mark as DRAW if actually proven (not timeout)
        tt_result = RESULT_EXACT_DRAW;
        move_result = RESULT_EXACT_DRAW;
    }
    tt_store(worker->global->tt, key, root->depth, root->pn, root->dn, tt_result);

    // DEBUG
    printf("DEBUG ROOT_SPLIT: move=%c%d pn=%u dn=%u tt_result=%s move_result=%s\n",
           'a' + (task->root_move % 8), 8 - (task->root_move / 8),
           root->pn, root->dn,
           tt_result == RESULT_EXACT_WIN ? "WIN" :
           (tt_result == RESULT_EXACT_LOSE ? "LOSE" :
           (tt_result == RESULT_EXACT_DRAW ? "DRAW" : "UNKNOWN")),
           move_result == RESULT_EXACT_WIN ? "WIN" :
           (move_result == RESULT_EXACT_LOSE ? "LOSE" :
           (move_result == RESULT_EXACT_DRAW ? "DRAW" : "UNKNOWN")));
    fflush(stdout);
    ...
    if (move_idx >= 0) {
        ...
        // else: UNKNOWNの場合は再キューしない
        // サブタスクとしてスポーンされた子ノードがTTに結果を保存し、
        // 通常のprocess_taskで再探索された時にTTヒットで結果が得られる
        // 再キューすると無限ループになるので、ここでは何もしない
    }

    free_dfpn_tree_children(root);
    node_pool_reset(&worker->node_pool);
    return true;
#endif  // ROOT_SPLIT disabled
}
...
static bool process_task(Worker *worker, Task *task) {
    ...
    // ルートタスク: task->playerが手番を持っているのでORノード
    // （ORノード = 勝つ手を1つ見つければ良い）
    // サブタスク: 保存されたnode_typeを使用
    NodeType type = task->is_root_task ? NODE_OR : task->node_type;
    ...
    // ★ Fix: For root tasks, loop until we get a deterministic result
    // This is similar to WPNS's iterative deepening approach
    int iteration = 0;
    int max_iterations = 1000;  // Prevent infinite loops

    while (iteration < max_iterations &&
           !worker->global->found_win &&
           !worker->global->shutdown) {
        iteration++;

        // Reset thresholds for each iteration
        root->threshold_pn = PN_INF + 1;
        root->threshold_dn = DN_INF + 1;
        ...
        if (root->pn == 0 || root->dn == 0) {
            break;  // Found WIN or LOSE
        }
        if (root->pn >= PN_INF && root->dn >= DN_INF) {
            break;  // DRAW
        }

        // For non-root tasks, one iteration is enough
        if (!task->is_root_task) {
            break;
        }

        // Re-probe TT for children to get any updates from parallel tasks
        if (root->children != NULL && root->n_children > 0) {
            bool any_update = false;
            for (int i = 0; i < root->n_children; i++) {
                DFPNNode *child = root->children[i];
                uint64_t child_key = hash_position(child->player, child->opponent);
                uint32_t tt_pn = 0, tt_dn = 0;
                Result tt_result = RESULT_UNKNOWN;
                if (tt_probe(worker->global->tt, child_key, child->depth, &tt_pn, &tt_dn, &tt_result)) {
                    if ((tt_pn == 0 || tt_dn == 0) && child->pn > 0 && child->dn > 0) {
                        child->pn = tt_pn;
                        child->dn = tt_dn;
                        child->result = tt_result;
            ...
            if (any_update) {
                update_pn_dn(root);
                if (root->pn == 0 || root->dn == 0) {
                    break;  // Result now determined
                }
            }
        }

        // Small delay to allow parallel tasks to make progress
        // (without this, we might spin-wait too aggressively)
        usleep(100);  // 0.1ms
    }
    ...
    if (worker->should_abort_task) {
        ...
        tt_store(worker->global->tt, key, root->depth, root->pn, root->dn, RESULT_UNKNOWN);
    ...
    // TT用の結果（ツリー視点）と move_results用の結果（元プレイヤー視点）を分離
    // - ルートタスクではmake_move後でplayer/opponentがスワップされているため、
    //   move_resultsに設定する結果は反転が必要
    // - TTにはツリー視点（非反転）の結果を保存し、他タスクが正しく参照できるようにする
    Result tt_result = RESULT_UNKNOWN;      // TTに保存する結果（ツリー視点）
    Result move_result = RESULT_UNKNOWN;    // move_resultsに設定する結果（元プレイヤー視点）

    // まずツリー視点の結果を計算（全タスク共通）
    // pn=0: このノードのプレイヤーの勝ち
    // dn=0: このノードのプレイヤーの負け
    // 両方INF && is_proven: DRAW（未証明ならUNKNOWN）
    if (root->pn == 0) {
        tt_result = RESULT_EXACT_WIN;
    } else if (root->dn == 0) {
        tt_result = RESULT_EXACT_LOSE;
    } else if (root->pn >= PN_INF && root->dn >= DN_INF && root->is_proven) {
        // ★ Fix: Only mark as DRAW if actually proven
        tt_result = RESULT_EXACT_DRAW;
    }

    // ルートタスクの場合、move_results用に結果を反転
    if (task->is_root_task) {
        // ルートタスク: bitboardがスワップされているので結果を反転
        // pn=0: タスクのplayer（相手）の勝ち = 元のプレイヤーのLOSE
        // dn=0: タスクのplayer（相手）が勝てない
        //   - result=LOSE: 相手の負け = 元のプレイヤーのWIN
        //   - result=DRAW: 引き分け = DRAW→LOSE（WPNS互換）
        if (root->pn == 0) {
            move_result = RESULT_EXACT_LOSE;
        } else if (root->dn == 0) {
            // 相手が勝てない → 元のプレイヤーの結果を判定
            if (root->result == RESULT_EXACT_LOSE) {
                // 相手の負け → 元のプレイヤーの WIN
                move_result = RESULT_EXACT_WIN;
            } else {
                // DRAW or other → DRAW=LOSE で、元のプレイヤーの LOSE
                move_result = RESULT_EXACT_LOSE;
            }
        } else if (root->pn >= PN_INF && root->dn >= DN_INF && root->is_proven) {
            // 両方INF → DRAW → LOSE（WPNS互換）
            move_result = RESULT_EXACT_LOSE;
        }
    } else {
        // サブタスク: ツリー視点と同じ
        move_result = tt_result;
    }
    // 注: pn < INF && dn < INF なら UNKNOWN のまま（探索未完了）

    // Store result in TT for other workers to find (ツリー視点、非反転)
    tt_store(worker->global->tt, key, root->depth, root->pn, root->dn, tt_result);
    ...
    if (task->is_root_task) {
            ...
            if (move_result != RESULT_UNKNOWN) {
                ...
                if (current == RESULT_UNKNOWN) {
                    ...
                    if (DEBUG_CONFIG.track_eval_impact && worker->global->eval_impacts) {
                        ...
                        worker->global->eval_impacts[move_idx].result = move_result;
                ...
                // UNKNOWNの場合も tasks_completed をインクリメント
                // （メインループがすべてのタスク処理完了を検知するため）
                atomic_add_int(&worker->global->tasks_completed, 1);
            }
        }
    } else {
        ...
        // サブタスクはTTに結果を保存するだけで、直接move_resultsを設定しない
        // ルートタスクが再探索時にTTから結果を取得し、正しく判定する
        // サブタスクの結果を直接使うと、tree depthによってplayer視点が異なるため誤判定になる
    }
    ...
    if (DEBUG_CONFIG.track_work_stealing) {
        debug_log("Worker %d completed task: move=%c%d, move_result=%s, nodes=%llu\n",
               worker->id,
               'a' + (task->root_move % 8), 8 - (task->root_move / 8),
               move_result == RESULT_EXACT_WIN ? "WIN" :
               (move_result == RESULT_EXACT_LOSE ? "LOSE" : "UNKNOWN"),
...
Result solve_endgame(uint64_t player, uint64_t opponent, int num_threads,
    ...
    while(moves_copy) {
        ...
        Task task = {
            ...
            .node_type = NODE_OR,   // task->playerが手番（ルートタスクはORノード）
    ...
    // DEBUG: Print each move's result
#if !MINIMAL_OUTPUT
    printf("\n=== DEBUG: Move Results ===\n");
    printf("found_win=%d, winning_move=%d\n", global.found_win, global.winning_move);
    fflush(stdout);
#endif
    for (int i = 0; i < n_moves; i++) {
        int move = global.move_list[i];
#if !MINIMAL_OUTPUT
        const char *res = global.move_results[i] == RESULT_EXACT_WIN ? "WIN" :
                         (global.move_results[i] == RESULT_EXACT_LOSE ? "LOSE" :
                         (global.move_results[i] == RESULT_EXACT_DRAW ? "DRAW" : "UNKNOWN"));
        printf("  Move %c%d: %s\n", 'a' + (move % 8), 8 - (move / 8), res);
#endif
    }
#if !MINIMAL_OUTPUT
    printf("===========================\n\n");
    ...
    if (global.found_win) {
        ...
        for (int i = 0; i < n_moves; i++) {
            ...
            if (global.move_results[i] == RESULT_UNKNOWN && final_result == RESULT_UNKNOWN) {
                if (final_best_move == -1) {
                    final_best_move = global.move_list[i];
                }
            }
    ...
    printf("Total: %llu nodes in %.3f sec (%llu NPS)\n",
           (unsigned long long)total_nodes, elapsed,
           (unsigned long long)(total_nodes > 0 && elapsed > 0 ? total_nodes / elapsed : 0));
...
int main(int argc, char *argv[]) {
    // Force line-buffered stdout for proper output ordering
    setvbuf(stdout, NULL, _IOLBF, 0);

    if (argc < 2) {
        fprintf(stderr, "Usage: %s <pos_file> [threads] [time_limit] [options]\n", argv[0]);
        fprintf(stderr, "\nThis is the NO-EVAL WORK STEALING version (evaluation function disabled).\n");
        fprintf(stderr, "Threads parameter means fixed worker count.\n");
        fprintf(stderr, "\nDebug options:\n");
        ...
        fprintf(stderr, "\nExamples:\n");
        fprintf(stderr, "  Basic:   %s test.pos 8 30.0 -v -w\n", argv[0]);
        fprintf(stderr, "  40-core: %s test.pos 40 120.0 -v -w -G 5 -D 4 -S 6\n", argv[0]);
        fprintf(stderr, "  Bench:   %s test.pos 8 30.0 -d debug.log -c results.csv\n", argv[0]);
        return 1;
    }
    ...
    char *eval_path = "eval/eval.dat";  // no_eval版では未使用
    (void)eval_path;  // 警告抑制
    ...
    // no_eval版: argv[4]からオプション解析開始（eval_pathは不要）
    ...
    if (debug_enabled) {
        debug_init(log_file, verbose, track_threads, track_tree, real_time, track_ws);
    }
    ...
    // 評価関数なし版: use_evaluationは常にfalse
    bool use_evaluation = false;
    (void)eval_path;  // unused
